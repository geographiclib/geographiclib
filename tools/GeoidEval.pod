...
GeoidEval(1) -- look up geoid heights
...
B<GeoidEval> in positions on standard input and print out the corresponding
...
any of the formats accepted by GeoConvert(1).  (MGRS coordinates signify
the I<center> of the corresponing MGRS square.)  If the B<-z> option is
...
use geoid I<name> instead of the default C<egm96-5>.  See L<GEOIDS|geoids>.

=item B<-d>

read geoid data from I<dir> instead of the default.  See L<GEOIDS|geoids>.
...
cache the data bounded by I<south> I<west> I<north> I<east> in memory.  See
L<CACHE|cache>.
...
the last token on the input line is treating as a height (in meters)
above the geoid and the output echos the input line with the height
converted to height above ellipsoid.

=item B<--haetomsl>

the last token on the input line is treating as a height (in meters)
above the ellipsoid and the output echos the input line with the height
converted to height above geoid.

=item B<-v>

print information about the geoid to be printed on standard error before
...
time, e.g., C</usr/local/share/GeographicLib/geoids>.  This may changed
by setting the environment variable C<GEOID_PATH> or with the B<-d>
option.  Use the B<-v> option to ascertain the full path name of the
data file.
...
If B<-a> is specified all the geoid data is read; in the case of
C<egm2008-1>, this requires about 0.5 GB of RAM.  The B<-c> option allows
...
Override the compile-time default geoid name.

=item B<GEOID_PATH>

Override the compile-time default geoid path.
...
C<ERROR:> and causes B<GeoidEval> to return an exit code of 1.  However,
an error does not cause B<GeoidEval> to terminate; following lines will
be converted.
