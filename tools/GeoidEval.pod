=for comment $Id: GeoidEval.pod 6982 2011-02-22 16:26:11Z karney $
...
the input line with the height converted to height above ellipsoid (HAE).
...
C<GEOID_PATH> or with the B<-d> option.  Use the B<-v> option to
...
Override the compile-time default geoid name of C<egm96-5>.  If the
B<-n> I<name> option is used then I<name> takes precedence.

=item B<GEOID_PATH>

Override the compile-time default geoid path.  This is typically
C</usr/local/share/GeographicLib/geoids> on Unix or Linux systems and
C<C:/cygwin/usr/local/share/GeographicLib/geoids> on Windows systems.
If the B<-d> I<dir> option is used then I<dir> takes precedence.
