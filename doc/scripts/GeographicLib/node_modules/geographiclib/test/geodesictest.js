...
describe("GeographicLib", function() {
  describe("GeodesicTest", function () {
    var geod = g.WGS84, i, check_geod_inverse, check_geod_direct;
  ...
  describe("GeodesicSolve", function () {
    ...
    it("GeodSolve1", function() {
      var geod = g.WGS84,
          inv = geod.Direct(40.63972222, -73.77888889,
                            53.5, 5850e3);
      assert.approx(inv.lat2, 49.01467, 0.5e-5);
      assert.approx(inv.lon2, 2.56106, 0.5e-5);
      assert.approx(inv.azi2, 111.62947, 0.5e-5);
    });
    ...
    it("GeodSolve4", function() {
      ...
      var geod = g.WGS84,
          inv = geod.Inverse(36.493349428792, 0,
    ...
    it("GeodSolve5", function() {
      ...
      var geod = g.WGS84,
          inv = geod.Direct(0.01777745589997, 30, 0, 10e6);
      assert.approx(inv.lat2, 90, 0.5e-5);
      if (inv.lon2 < 0) {
        assert.approx(inv.lon2, -150, 0.5e-5);
        assert.approx(inv.azi2, -180, 0.5e-5);
      } else {
        assert.approx(inv.lon2, 30, 0.5e-5);
        assert.approx(inv.azi2, 0, 0.5e-5);
      }
    });
    ...
    it("GeodSolve28", function() {
      ...
      var geod = new g.Geodesic(6.4e6, 0),
          dir = geod.Inverse(1, 2, 3, 4, g.AREA);
      assert.approx(dir.S12, 49911046115.0, 0.5);
    });
