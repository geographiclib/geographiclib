#! /bin/sh
for x in net ""; do
while read package libtype cmakeversion stat; do
    builddir=/tmp/${x}geogtest-$package-$libtype-$cmakeversion
    rm -rf $builddir
    mkdir $builddir
    cd $builddir
    case $cmakeversion in
	old ) test "$x" && continue;cmake=/u/temp/cmake-2.8.0/bin/cmake;;
	new ) cmake=cmake;;
    esac
    case $libtype in
	shared ) static=OFF;;
...
geoginst0a static new
geoginst0a static old
geoginst1  shared new
geoginst1  shared old
geoginst1  static new
geoginst1  static old
geoginst1a static new
geoginst1a static old
EOF
done
exit
