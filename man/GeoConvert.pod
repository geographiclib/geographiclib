...
geographic coordinates.  If the coordinate includes an interior sign
(i.e., not at the beginning or immediately after an initial hemisphere
designator), then the coordinate is split before that sign; the two
pieces are parsed separately and the results summed.  For example the
