...
be given on the command line as the argument to the B<-t> option, in
...
via the B<-c> option; in this case only the longitude should be given on
the input lines.
...
evaluate the field on a circle of latitude at I<time>, I<lat>, I<h>
instead of reading these quantities from the input lines.  In this case,
B<MagneticField> can calculate the field considerably quicker.
