...
B<RhumbSolve> [ B<-i> | B<-l> I<lat1> I<lon1> I<azi> ]
[ B<-e> I<a> I<f> ]
[ B<-d> | B<-:> ] [ B<-f> ] [ B<-p> I<prec> ] [ B<-E> ]
...
(I<lat1>, I<lon1>) and (I<lat2>, I<lon2>) is called the rhumb line.  Its
length is I<s12> and the rhumb line has a forward azimuth I<azi> along
its length.  The longitude becomes indeterminate when a rhumb line
passes through a pole, and B<RhumbSolve> reports NaNs for the positions
in this case.  <B>NOTE:</B> the rhumb line is <B>not</B> the shortest
path between two points; that is the geodesic and it is calculated by
GeodSolve(1).
...
containing I<lat1> I<lon1> I<azi> I<s12> and prints I<lat2> I<lon2> on
standard output.  This is the direct calculation.

=item 2.

Command line arguments B<-l> I<lat1> I<lon1> I<azi> specify a rhumb
...
With the B<-i> command line argument, B<RhumbSolve> performs the
inverse calculation.  It reads lines containing I<lat1> I<lon1>
I<lat2> I<lon2> and prints the corresponding values of I<azi> I<s12>.
...
rhumb line specified by I<lat1> I<lon1> I<azi>.
...
By default, a series expansion is used to compute a key ratio of
differences accurately.  The series are only accurate for |I<f>| < 0.01.
If B<-E> is supplied, the ratio is computed directly using exact
expressions.  However this calculation is subject to large round-off
errors if I<lat1> and I<lat2> are close.  See L</ACCURACY>.
...
flattening.  However the computation of rhumb lines involves the ratio
(I<psi1> - I<psi2>) / (I<mu1> - I<mu2>) and this is subject to large
round-off errors if I<lat1> is close to I<lat2>.  So, by default, the
ratio is computed using divided differences using the series expansions
used by TransverseMercatorProj(1) for the conversions between rectifying
and conformal latitudes.  These are accurate for |I<f>| < 0.01.  If the
B<-E> option is supplied then the ratio of the differences will be
computed directly if I<lat1> is far from I<lat2> and will be
approximated by the derivative otherwise.  This results in appreciable
errors (about 0.1 mm) if I<lat1> and I<lat2> are within a few
arcseconds.  Recommendation: do not supply the B<-E> option for
terrestrial applications; for highly eccentric ellipsoids, use B<-E> and
be suspicious of the result if I<lat1> and I<lat2> are close.
