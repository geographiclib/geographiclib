...
addition theorem for elliptics integrals (this is accurate for all
values of I<f>); however, with the B<-s> options, it is computed using
the series expansions used by TransverseMercatorProj(1) for the
conversions between rectifying and conformal latitudes (these are
accurate for |I<f>| E<lt> 0.01).  For the WGS84 ellipsoid the error is
about 10 nanometers using either method.
