...
its length.  A point at a pole is treating as a point a tiny distance
...
B<RhumbSolve> reports NaNs for the longitude in this case.

<B>NOTE:</B> the rhumb line is <B>not</B> the shortest path between two
points; that is the geodesic and it is calculated by GeodSolve(1).
