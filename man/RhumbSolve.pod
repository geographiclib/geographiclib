...
RhumbSolve -- perform geodesic calculations
...
expansions for |I<f>| > 0.02.  However they are subject to greater
round-off errors if I<lat1> and I<lat2> are close.
...
The algorithm used by B<RhumbSolve> applies for any value of the
flattening.  However, the accuracy is substantially less then
GeodSolve(1), particularly if I<lat1> and I<lat2> are within a few
arcseconds.  Typically the errors in the distance are less than 0.1 mm.
The accuracy could be improved for small flattening by using a series
for the rectifying latitude and by using divided differences to compute
the ratio of the changes in the isometric and rectifying latitudes.
