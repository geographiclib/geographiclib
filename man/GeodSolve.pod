...
in terms of direct or inverse geodesic calculation.
...
provided that it appears before B<-D>.  Similarly the B<-a> can be uses
to change the interpretation of I<s13> to I<a13>.
...
options.  In this case, the values read on standard input are
interpreted as fractional distances to point 3, i.e., I<s12>/I<s13>.  If
arc mode is in effect, then values are fractional arc length, i.e.,
I<a12>/I<a13>.
...
B<GeodSolve> was added to GeographicLib, L<http://geographiclib.sourceforge.net>,
