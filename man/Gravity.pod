...
changed by setting the environment variable C<GEOGRAPHICLIB_GRAVITY_NAME> or with the
B<-n> option.
...
C<GEOGRAPHICLIB_GRAVITY_PATH> or C<GEOGRAPHICLIB_DATA>, or with the B<-d> option.
...
option reports the value of B<GEOGRAPHICLIB_GRAVITY_NAME>, if defined, otherwise it
