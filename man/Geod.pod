...
Using the (default) series solution, Geod is accurate to about 15 nm (15
...
the WGS84 ellipsoid and different values of the flattening is
...
If B<-E> is specified, Geod is accurate to about 40 nm (40 nanometers)
...
different values of the I<a/b> = 1 - I<f> is
...
=item I<lat1> = -I<lat2> (with neither at a pole)
...
=item I<lon2> = I<lon1> +/- 180 (with neither at a pole)
...
when the I<lat2> is near -I<lat1> for prolate ellipsoids.)
...
The proj library has a utility B<geod> which offers similar
functionality (but which uses less accurate algorithms).  B<Geod> and
B<geod> are distinct names on most operating systems, but can be
confused on Windows systems.  If you have proj installed on your Windows
system, then in order to use B<Geod> you will need to use the full
pathname for the executable or ensure that your C<PATH> includes the
GeographicLib binary directory before the proj directory.

=head1 SEE ALSO

GeoConvert(1).  The algorithms are described in C. F. F. Karney,
I<Algorithms for geodesics>, J. Geodesy, 2012; DOI:
