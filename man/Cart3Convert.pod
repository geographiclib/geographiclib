...
B<Cart3Convert> [ B<-E> | B<-G> | B<-P> | B<-C> | B<-GX> | B<-PX> | B<-CX> ]
[ B<-r> ] [ B<-3> | B<-D> ]
[ B<-R> I<nrand> ] [ B<--seed> I<s> ]
...
semiaxes, I<a>, I<b>, and I<c>.  By default, the conversion is to
...
Ellipsoidal coordinates, I<beta>, I<omega>, and I<alpha>, signified by
B<-E>n (the default)

=item 2.

Geodetic coordinates, I<phi>, I<lambda>, and I<zeta>, signified by
B<-G>.

=item 3.

Parametric coordinates, I<phi'>, I<lambda'>, and I<zeta>, signified by
B<-P>.

=item 4.

Geocentric coordinates, I<phi">, I<lambda">, and I<zeta>, signified by
B<-C>.

=item 5.

Geodetic coordinates, I<phi*>, I<lambda*>, and I<zeta*>, with the north
pole lying on the I<X> axis, signified by B<-GX>.

=item 6.

Parametric coordinates, I<phi*'>, I<lambda*'>, and I<zeta*>, with the
north pole lying on the I<X> axis, signified by B<-PX>.

=item 7.

Geocentric coordinates, I<phi*">, I<lambda*">, and I<zeta*>, with the
north pole lying on the I<X> axis, signified by B<-CX>.
...
clockwise from a line of constant longitude.  I<zeta> is the same for
...
degrees or degrees, minutes and seconds).  The interpretation of the
coordinates is governed by the B<-E>, B<-G>, B<-P>, B<-C> B<-GX>,
B<-PX>, and B<-CX> flags.  For each set of coordinates, the
corresponding cartesian coordinates I<X>, I<Y>, I<Z> (meters) are
printed on standard output.  If I<-r> is provided, the reserve operation
is undertaken reading I<X>, I<Y>, I<Z> (which are assumed to denote a
point on the ellipsoid) on standard input and printing the I<latitude>
and I<longitude>.

If the B<-3> flag is given (this cannot be combined with B<-D>), an
arbitrary point in three-dimensional space is specified either with
I<beta> I<omega> I<H> or with I<phi> I<lambda> I<h>.

If the B<-D> flag is given (this cannot be combined with B<-3>), the
heading (direction) is included in the computation converting I<beta>
I<omega> I<alpha> or I<phi> I<lambda> I<zeta> to I<X>, I<Y>, I<Z> I<Vx>,
I<Vy>, I<Vz> where [I<vx>, I<vy>, I<vz>] is a unit vector lying in the
...
I<latitude> and I<longitude> is interpreted as ellipsoidal coordinates
I<beta> and I<omega>.  This is the default.

=item B<-G> or B<-GX>

I<latitude> and I<longitude> is interpreted as geodetic coordinates
I<phi> and I<lambda>.  The B<X> added to the flags means that the north
pole lies on the I<X> axis.

=item B<-P> or B<-PX>

I<latitude> and I<longitude> is interpreted as parametric coordinates
I<phi'> and I<lambda'>.  The B<X> added to the flags means that the north
pole lies on the I<X> axis.

=item B<-C> or B<-CX>

I<latitude> and I<longitude> is interpreted as geocentric coordinates
I<phi"> and I<lambda">.  The B<X> added to the flags means that the north
pole lies on the I<X> axis.
...
include the height I<h> or I<H> in the calculation.  This cannot be
combined with B<-D>.

=item B<-D>

include the direction I<alpha> or I<zeta> in the calculation.  With the
B<-r> option, it is assumed that the vector [I<vx>, I<vy>, I<vz>] is
...
cartesian coordinates.  With B<-D> add also a random cartesian direction
tangent to the surface of the ellipsoid.  In this mode, nothing is read
from standard input.  With the B<-r> option the random points are
converted into one of the triaxial coordinate systems (and the direction
is converted to a heading if B<-D> is given).

=item B<--seed> I<s>

set the random number seed to I<s> when using B<-R>.  I<s> = 0 (the
...
specify the ellipsoid via the median semiaxis, I<b> and the shape
...
The algorithms are described in Appendices A and B of
C. F. F. Karney,
I<Jacobi's solution for geodesics on a triaxial ellipsoid>,
J. Geodesy 100, 17:1-23 (2026); DOI:
L<https://doi.org/10.1007/s00190-026-02030-3>.
