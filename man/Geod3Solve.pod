...
precision, I<prec> = 3 giving 1 mm precision, etc.  For decimal
degrees, the number of digits after the decimal point is I<prec> + 5.
For DMS (degree, minute, seconds) output, the number of digits after
the decimal point in the seconds component is I<prec> + 1.  For
distances, I<prec> is the number of digits after the decimal point for
ellipsoids which are approximately the same size as the Earth; for
other ellipsoids the precision is adjusted to retain the same relative
precision. The minimum value of I<prec> is 0 and the maximum is 10.
...
= [I<azi2>,I<azi1>].  (This occurs when the longitude difference is
near +/-180 for oblate ellipsoids.)
...
[I<azi1>,I<azi2>] = [-I<azi1>,-I<azi2>].  (This occurs when I<lat2> is
near -I<lat1> for prolate ellipsoids.)
