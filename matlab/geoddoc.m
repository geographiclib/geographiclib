...
%   sqrt(a^2 - b^2)/a, and b is the polar semi-axis.  Typically, a and b
%   are measured in meters and the linear and area quantities returned by
%   the routines are then in meters and meters^2.  However, other units can
%   be employed.  If ELLIPSOID is omitted, then the WGS84 ellipsoid is
%   assumed [6378137, 0.0818191908426215] corresponding to a = 6378137
...
%   cases:
%     * lat1 = -lat2 (with neither at a pole).  If azi1 = azi2, the
%       geodesic is unique.  Otherwise there are two geodesics and the
%       second one is obtained by setting [azi1,azi2] = [azi2,azi1].  (This
%       occurs when the longitude difference is near +/-180 for oblate
%       ellipsoids.)
%     * lon2 = lon1 +/- 180 (with neither at a pole).  If azi1 = 0 or
%       +/-180, the geodesic is unique.  Otherwise there are two geodesics
%       and the second one is obtained by setting [azi1,azi2] =
%       [-azi1,-azi2].  (This occurs when the lat2 is near -lat1 for
%       prolate ellipsoids.)
...
% This file was distributed with GeographicLib 1.27.
  ...
  fprintf('type ''help geoddoc'' for documentation\n');
end
