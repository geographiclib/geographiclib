...
namespace GeographicLib {
  ...
  public:
     ...
     * not all) components may be omitted and other symbols (e.g., the
     * <sup>o</sup> symbol) to indicate these components.  The last component
     ...
     * may be used to <i>separate</i> these components (numbers must appear
     * before and after each colon); thus 50d30'10.3&quot; may be written as
     * 50:30:10.3, 5.5' may be written 0:5.5, and so on.  The integer parts of
     * the minutes and seconds components must be less than 60.  A single
     * leading sign is permitted.  A hemisphere designator (N, E, W, S) may be
     * added to the beginning or end of the string.  The result is multiplied
     * by the implied sign of the hemisphere designator (negative for S and W).
     * In addition \e ind is set to DMS::LATITUDE if N or S is present, to
     * DMS::LONGITUDE if E or W is present, and to DMS::NONE otherwise.  Throws
     * an error on a malformed string.  No check is performed on the range of
     * the result.  Examples of legal and illegal strings are
