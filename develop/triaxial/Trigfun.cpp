...
namespace GeographicLib {
  ...
  Trigfun::Trigfun(const function<real(real)>& f, bool odd, bool sym,
                   bool centerp, real halfp, int n, real tol) {
    if (n == 0) {
      int n = 16;
      Trigfun t(f, odd, sym, false, halfp, n);
      while (n < (1 << 17)) {
        int K = t.chop(t._coeff, tol)
