...
namespace GeographicLib {
  ...
  PolarStereographic::PolarStereographic(double a, double invf, double k0)
    ...
    , _e(std::sqrt(_f * (2 - _f)))
    , _e2m(1 - sq(_e))
    , _c(std::sqrt( std::pow(1 + _e, 1 + _e) * std::pow(1 - _e, 1 - _e) ))
    , _tol(0.1*std::sqrt(std::numeric_limits<double>::epsilon()))
    , _numit(5)
  {}
  ...
  void PolarStereographic::Forward(bool northp, double lat, double lon,
    ...
    double
      ecos = _e * std::cos(theta),
      f = std::pow((1 + ecos)/(1 - ecos), _e/2),
      t2 = 2 * std::tan(theta/2) * f,	   // Snyder (15-9) (t2 = 2 * t)
      m = std::sin(theta) / std::sqrt(1 - sq(ecos)); // Snyder (14-15)
    rho = _a * _k0 * t2 / _c;		   // Snyder (21-33)
    k = m < std::numeric_limits<double>::epsilon() ? _k0 : rho / (_a * m);
    double
      lam = lon * Constants::degree;
    x = rho * std::sin(lam);
    y = (northp ? -rho : rho) * std::cos(lam);
  ...
  void PolarStereographic::Reverse(bool northp, double x, double y,
    ...
    lon = rho == 0 ? 0 : -std::atan2( -x, northp ? -y : y ) / Constants::degree;
    double
      m = std::sin(theta) / std::sqrt(1 - sq(ecos));
    k = m == 0 ? _k0 : rho / (_a * m);
    gamma = northp ? lon : -lon;
  }
